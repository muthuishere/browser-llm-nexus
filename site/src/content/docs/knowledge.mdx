...
`chat` and `embedder` each accept either a [model source](/browser-llm-nexus/model-sources/)
...
- [Embeddings & RAG](/browser-llm-nexus/embeddings-rag/) — the pieces underneath, used directly
- [Where models come from](/browser-llm-nexus/model-sources/) — `{ archive }` as a load source
